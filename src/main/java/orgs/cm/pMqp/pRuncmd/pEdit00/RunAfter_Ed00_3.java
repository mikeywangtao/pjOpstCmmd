...
public class RunAfter_Ed00_3 extends AbsRunAfter {


	private AbsDbpro objDbpro = null;
	private HashMap<String, Object> hmpAll;
	private final String strCname = RunAfter_Ed00_3.class.getName();
...
//		ArrayList<LinkedHashMap<String, String>> altRunc = new ArrayList<LinkedHashMap<String, String>>();	
		...
		try {
			...
			if(hmpAll.size()>0) {
				...
				strInfo = strCname + strFname + " 编辑VM After01 Start----" + DatePro.disGetStrdate4NowObjSdf001();
...
//				hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
				...
				if(hmpAll.containsKey(ProcessAttrs.strInfoFlgKey_Resstd)
					...
					if(strDevname!=null && strDevname.trim().length()>0){
						...
						ResFormatpro_Ed003 objResFormatpro_3 = new ResFormatpro_Ed003(hmpAll, altStd, strDevname, objBa);
						String strFlg = objResFormatpro_3.disGetFlg();
						if(strFlg!=null && strFlg.trim().length()>0){
//						if(subFlg!=null && subFlg.length==2){
							if(strFlg.indexOf("ACTIVE")>-1
									&& strFlg.indexOf("status")>-1
									&& strFlg.indexOf("flavor")>-1){
//							if(!"ACTIVE".equals(subFlg[0])){
								String[] subFlg = strFlg.split(",,,");
								String strFlvid = "";
								for(String str : subFlg){
									if(str.indexOf("flavor")>-1){
										strFlvid = str.split("}}}")[0];
									}
								}
								...
								hmpAll.put("strFlavor", strFlvid);
//								hmpAll.put("^devids^", subFlg[1]);
								((HashMap<String, String>)hmpAll.get(ProcessAttrs.strParmapKey_Inpars)).put("^strFlavor^", strFlvid);
								strInfo = strCname + strFname + " 编辑VM After01 ----strParmapKey_Ppa_NowRunflg false" + DatePro.disGetStrdate4NowObjSdf001();
								...
								hmpAll.put("strVmIp", hmpAll.get("^vmids^")==null? "":hmpAll.get("^vmids^").toString());
								hmpAll.put("strFlavor", "");
								strInfo = strCname + strFname + " 编辑VM After01 ----strParmapKey_Ppa_NowRunflg true" + DatePro.disGetStrdate4NowObjSdf001();
				...
				if(hmpAll.containsKey(ProcessAttrs.strInfoFlgKey_Reserr)
					...
					strInfo = strCname + strFname + " 编辑VM After01 ----strInfoFlgKey_Reserr Error" + DatePro.disGetStrdate4NowObjSdf001();
...
//				DbInfotablePro4Cmmd.disInfotablePro(disGetBusname());
				...
				strRes = strRes +  "," + strCname + strFname+ "编辑VM Run01after ok ----end " ;
				hmpAll.put(ProcessAttrs.strParmapKey_Ppa_RunResLst, strRes);				strInfo = strCname + strFname + " 编辑VM After01 End----" + DatePro.disGetStrdate4NowObjSdf001();
				objBa.altRunc = objBa.objSetInfoPro.disSetInfo_000(strInfo, objBa.lhpInfobase, objBa.altRunc, ProcessAttrs.strInfoFlg_PRE);
//				hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
			}
		} catch(Exception ex) {
			String strRes = ((String)hmpAll.get(ProcessAttrs.strParmapKey_Ppa_RunResLst));
			strRes = strRes +  "," + strCname + strFname+ "编辑VM Run01after Ex " + ex ;
