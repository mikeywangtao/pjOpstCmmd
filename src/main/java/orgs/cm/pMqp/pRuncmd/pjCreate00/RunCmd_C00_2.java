...
//		ArrayList<LinkedHashMap<String, String>> altRunc = new ArrayList<LinkedHashMap<String, String>>();		
		try {
			...
			lhpInfobase.put(ProcessAttrs.strInfoType_Info, ProcessAttrs.strInfoFlgKey_Runc);
			strInfo = strCname + strFname + " 创建VM Run02 Start----" + DatePro.disGetStrdate4NowObjSdf001();
...
//			StrCommand = StrCommand.substring(0, StrCommand.length()-1);
			strInfo = strCname + strFname + " 创建VM Run02 Cmmd----" + StrCommand;
			strInfo = strInfo.replaceAll("'", "\"");
			altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, null);
			logger.info(strInfo);
			...
			if(errorGobbler!=null && outputGobbler!=null){
				...
				super.strThrflg = null;
				strInfo = strCname + strFname + " 创建VM Run02 End----" + DatePro.disGetStrdate4NowObjSdf001();
