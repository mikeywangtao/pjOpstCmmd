...
//				hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
				...
				if(strAnsidf!=null && strAnsidf.trim().length()>0){
					...
					if(strFlg!=null && strFlg.trim().length()>0){
						...
						if(subFlg!=null && subFlg.length==2){
							...
							altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, ProcessAttrs.strInfoFlg_PRx + " booResCheck true ");
							hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
						} else {
							...
							altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, ProcessAttrs.strInfoFlg_Elx + " disResCheck return error ");
							hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
						}
					} else {
						hmpAll.put(ProcessAttrs.strParmapKey_Ppa_NowRunflg, null);
						strInfo = strCname + strFname + " VM创建 After03 ----disResCheck return null" + DatePro.disGetStrdate4NowObjSdf001();
						altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, ProcessAttrs.strInfoFlg_Elx + " disResCheck return null ");
						hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
					}
				}
				...
				((ArrayList<String>)hmpAll.get(ProcessAttrs.strParmapKey_Ppa_RunResLst)).add(strCname + strFname + " Run03after ok ----end ");
				strInfo = strCname + strFname + " VM创建 After03 End----" + DatePro.disGetStrdate4NowObjSdf001();
