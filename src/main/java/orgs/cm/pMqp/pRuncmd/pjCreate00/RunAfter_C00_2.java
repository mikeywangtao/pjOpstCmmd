...
//		ArrayList<LinkedHashMap<String, String>> altRunc = new ArrayList<LinkedHashMap<String, String>>();	
		...
		try {
			if(hmpAll!=null && hmpAll.size()>0) {
				...
				strInfo = strCname + strFname + " VM创建 After02 Start----" + DatePro.disGetStrdate4NowObjSdf001();
...
//				hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
					...
					if(strDevname!=null && strDevname.trim().length()>0){
						...
						if(subFlg!=null && subFlg.length==2){
							if("available".equals(subFlg[0])){
								...
								strInfo = strCname + strFname + " VM创建 After02 ----strParmapKey_Ppa_NowRunflg false" + DatePro.disGetStrdate4NowObjSdf001();
								altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, ProcessAttrs.strInfoFlg_PRx + " strParmapKey_Ppa_NowRunflg false ");
								hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
							} else {
								hmpAll.put(ProcessAttrs.strParmapKey_Ppa_RunLoopFlg, "t");//不循环runcmd
								hmpAll.put(ProcessAttrs.strParmapKey_Ppa_NowRunflg, "2");
								strInfo = strCname + strFname + " VM创建 After02 ----strParmapKey_Ppa_NowRunflg true" + DatePro.disGetStrdate4NowObjSdf001();
								altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, ProcessAttrs.strInfoFlg_PRx + " strParmapKey_Ppa_NowRunflg true ");
								hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
							}
						}
					}
				...
				if(hmpAll.containsKey(ProcessAttrs.strInfoFlgKey_Reserr)
					...
					strInfo = strCname + strFname + " VM创建 After02 ----strInfoFlgKey_Reserr Error" + DatePro.disGetStrdate4NowObjSdf001();
					altRunc = disSetInfo(strInfo, lhpInfobase, altRunc, ProcessAttrs.strInfoFlg_Elx + " strInfoFlgKey_Reserr Error ");
//					hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, altRunc);
				}
