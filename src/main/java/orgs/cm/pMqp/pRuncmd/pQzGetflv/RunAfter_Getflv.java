...
//	private AbsDbpro objDbpro = null;
	...
	private final String strCname = RunAfter_Getflv.class.getName();
...
//		ArrayList<LinkedHashMap<String, String>> altRunc = new ArrayList<LinkedHashMap<String, String>>();	
		...
		try {
			...
			if(hmpAll.size()>0
				...
				strInfo = strCname + strFname + " 系统模板 After Start----" + DatePro.disGetStrdate4NowObjSdf001();
					...
					if(strAnsidf!=null && strAnsidf.trim().length()>0){
						...
						if(altResf!=null){
							...
							if(strReq!=null && strReq.trim().length()>0){
								...
								strInfo = strCname + strFname + " 系统模板 After RequestBody----" + strSetImg;
								objBa.altRunc = objBa.objSetInfoPro.disSetInfo_000(strInfo, objBa.lhpInfobase, objBa.altRunc, null);
								hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, objBa.altRunc);
								HttpClientUtil objHttpClientUtil = new HttpClientUtil();
								logger.info(strInfo);
								String strSetImgres = objHttpClientUtil.sendHttpPostJson("http://"+strRemoteSer+":"+strReSerpoint+"/pjOpStAuth/web/template/saveTemplate", strSetImg);
//								String strSetImgres = objHttpClientUtil.sendHttpPostJson("http://10.167.212.105:9001/pjOpStAuth/web/images/saveImages", strSetImg);
								...
								strInfo = strCname + strFname + " 系统模板 After respones----" + mapResAnsible;
								logger.info(strInfo);
								objBa.altRunc = objBa.objSetInfoPro.disSetInfo_000(strInfo, objBa.lhpInfobase, objBa.altRunc, null);
								hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, objBa.altRunc);
							}
				...
				strInfo = strCname + strFname + " 系统模板 After End----" + DatePro.disGetStrdate4NowObjSdf001();
				objBa.altRunc = objBa.objSetInfoPro.disSetInfo_000(strInfo, objBa.lhpInfobase, objBa.altRunc, null);
				hmpAll.put(ProcessAttrs.strParmapKey_Aftlst, objBa.altRunc);
			}
		} catch(Exception ex) {
