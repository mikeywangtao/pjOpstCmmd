...
public class RunBefore_RelDe00_1 extends AbsRunBefore {

	private final String strCname = RunBefore_RelDe00_1.class.getName();
				...
				if(subCmmd!=null && subCmmd.length==6 //命令数变化需要修改
					...
					for(int i=0; i<altCmdsh.size(); i++){
						...
						for(int j=0; j<altCmdpar.size(); j++){
							...
							System.out.println("strSigParam.replaceAll(  value : " + strSigParam.replaceAll("\\^", "\\\\^"));
							System.out.println("mapParam.get(strSigParam)  value : " + mapParam.get(strSigParam));
