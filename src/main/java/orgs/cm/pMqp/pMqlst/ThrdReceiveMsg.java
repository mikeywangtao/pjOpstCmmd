...
public class ThrdReceiveMsg extends AbsThrdReceiveMsg {
	...
	private String STOMPIP = null; // ActiveMQConnection.DEFAULT_BROKER_URL;//默认连接地址
	...
	public void run() {
		...
		try {
			// if(disCreate()){
			disGetProperties();
			if (strServerTarget != null && strServerTarget.trim().length() > 0 && strFlg != null
					&& strFlg.trim().length() > 0) {
				StompConnection connection = null;
				connection = new StompConnection();
				connection.open(STOMPIP, 61613);
				connection.connect(USERNAME, PASSWORD);
				connection.subscribe("/queue/"+strFlg+"_"+strServerTarget, Subscribe.AckModeValues.AUTO);
...
//						message = connection.receive(waitTimeOut);
						...
						if(ex.getMessage().indexOf("Read timed out")!=0){
							long lonFlg = System.currentTimeMillis();
							logger.error(strCname + strFname + ex + "||" + lonFlg);
							StackTraceElement[] subSte = ex.getStackTrace();
							for (int i = 0; i < subSte.length; i++) {
								logger.error(subSte[i].getClassName() + subSte[i].getMethodName() + ":" + subSte[i].getLineNumber()
										+ "||" + lonFlg);
							}
						}
					}
				...
				// destination =
				// session.createQueue(strFlg+"_"+strServerTarget);
				// // 创建消息消费者
...
//	private void disReveivePro(MessageConsumer messageConsumer) {
	private void disReveivePro(StompFrame message) {
		...
		try {
			...
			System.out.println("message.meesage_id = " + meesage_id);
			System.out.println("message.getBody() = " + message.getBody());
			System.out.println("message.getCommandId() = " + message.getCommandId());
...
//			}
	...
	private void disGetProperties() throws Exception {
		...
		try {
			...
			objStompip = Propertiesmap.getPptAttr("activeMq.properties", "stompip");
			...
			this.STOMPIP = objStompip==null? null:objStompip.toString();
			// if(PropetiesManage.pptMq!=null){
			// this.USERNAME = null;
			// //PropetiesManage.pptMq.getProperty("USERNAME");// 默认连接用户名
			// //admin
			// this.PASSWORD = null;
			// //PropetiesManage.pptMq.getProperty("PASSWORD");// 默认连接密码
			// //admin123456
			// this.BROKEURL =
			// "failover:"+PropetiesManage.pptMq.getProperty("BROKEURL");
			// //"failover://tcp://"+PropetiesManage.pptMq.getProperty("BROKEURL");//
			// 默认连接地址 //"http://10.167.210.212:8161/admin/"
			// }
			// this.USERNAME = ActiveMQConnection.DEFAULT_USER;// 默认连接用户名
			// //admin
			// this.PASSWORD = ActiveMQConnection.DEFAULT_PASSWORD;// 默认连接密码
			// //admin123456
			// this.BROKEURL = ActiveMQConnection.DEFAULT_BROKER_URL;// 默认连接地址
			// //"http://10.167.210.212:8161/admin/"
		} catch (Exception ex) {
			long lonFlg = System.currentTimeMillis();
			logger.error(strCname + strFname + ex + "||" + lonFlg);
			StackTraceElement[] subSte = ex.getStackTrace();
	...
	// strPath = strPath.substring(0, intOdx) +
	...
	// int intOdx = strPath.indexOf("WEB-INF");
	// strPath = strPath.substring(0, intOdx) +
	// "WEB-INF/classes/Mngdb.properties";
	// GetProperties objPro = new GetProperties();
	...
	// ThreadPoolManage.chmThdPool_Msg.put(strFlg+strServerTarget, new
	// ThreadPool01(500, ThrdSaveMsg.class));
	// ThreadRunManage.chmthdrMang_DataSave.put(strFlg+strServerTarget, new
	// ThreadRun01(intPool));
	// booRe = true;
	// }
	// }
